...
class Spec extends Backend
{
        ...
        if ($this->request->isPost())
        {
            ...
            if ($params)
            {
                ...
                try
                {
                    ...
                    foreach ($items as $v) {
                        $specItem[] = [
                            'dish_spec_id' => $this->model->id,
                            'name' => $v
                        ];
                    }
                    $result2 = model('DishSpecItem')->saveAll($specItem);
        ...
        if ($this->request->isPost())
        {
            ...
            if ($params)
            {
                try
                {
                    ...
                    $items = $params['item'];
                    $items = explode("\n",$items);
                    $specItems = model("DishSpecItem")->where('dish_spec_id',$ids)->column('name');
                    ...
                    $result2 = model('DishSpecItem')->saveAll($specItem);
                    $result3 = model('DishSpecItem')->where('dish_spec_id',$ids)->where('name','in',$specItmeDelete)->delete();
    ...
    public function del($ids = "")
    {
        if ($ids)
        {
            ...
            model('DishSpecItem')->where('dish_spec_id','in',$ids)->delete();
            $count = 0;
            foreach ($list as $k => $v)
            {
                $count += $v->delete();
